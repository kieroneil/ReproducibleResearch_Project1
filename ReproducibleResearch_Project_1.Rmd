...
*Note that NAs occur during an entire day,
...
### NAs omitted
```{r}
ggplot(total_steps_na_omit, aes(total_steps)) + geom_histogram(bins = 15)
```

## 3. Mean and median number of steps taken each day
Calculate and report the mean and median of the total number of steps taken per day with NA records omitted.  
```{r}
...
```{r}
ggplot(total_steps_imputed, aes(total_steps)) + geom_histogram(bins = 15)
```
